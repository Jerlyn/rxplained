...
**Homepage had no Open Graph tags at all, August 2026**: `index.html` never got the same treatment as the per-term pages above — it had a plain `<meta name="description">` and nothing else, so LinkedIn (and any other crawler) had no `og:` tags to build a preview card from, hence the "content is blocked" placeholder LinkedIn shows when it can't unfurl a link. Confirmed via `curl` (matching what a non-JS-executing crawler actually sees) that this was a missing-metadata gap, not a `robots.txt` block — `robots.txt` allows everything. Added static `og:`/`twitter:` tags directly to `index.html`'s `<head>`, alongside the existing description tag — no JS injection needed, since it's the same content on every homepage visit. Also created a real 1200×630 social-preview image (`images/og-image.png`) matching the site's actual palette, logo treatment, and headline copy, rather than reusing the square `icon-512.png` the way the per-term pages currently do as a stopgap (see *Known gaps* below) — a square image gets cropped oddly in a wide card layout. Deliberately did not touch `scripts/generate_term_pages.py` or regenerate `term/`, and deliberately did not add the new image to `sw.js`'s offline precache list, since it's never rendered inside the app itself — only ever fetched by crawlers. **Not verified against the live site**: confirming with LinkedIn's Post Inspector (linkedin.com/post-inspector) and forcing a re-crawl to clear any cached "blocked" result has to happen after this deploys — do that once it's live.
